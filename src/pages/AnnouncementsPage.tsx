...
import { Megaphone, Bell, Info, ArrowLeft, Calendar, ShieldCheck, User as UserIcon, Trash2, Plus, X, AlertCircle, Camera, Image as ImageIcon } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import { db } from '../firebase/config';
import { collection, query, onSnapshot, addDoc, deleteDoc, doc, serverTimestamp, where, getDocs, updateDoc } from 'firebase/firestore';
...
export default function AnnouncementsPage({ isAdminView = false, user = null }: AnnouncementsPageProps) {
  ...
  const handleAddAnnouncement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.accountType === 'admin') {
        ...
        else if (user.adminRole === 'rt') authorRole = `Ketua RT 00${user.rt_id}`;
  ...
  const handleDeleteAnnouncement = (id: string) => {
    if (id.startsWith('mock-')) {
      alert("Item demo mock tidak dapat dihapus dari database.");
      return;
    }
    ...
    >
      ...
      <header style={{ marginBottom: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <button 
            onClick={() => navigate(isAdminView ? '/admin/dev/dashboard' : '/warga/dashboard')}
            style={{
              width: 40,
              height: 40,
              borderRadius: 12,
